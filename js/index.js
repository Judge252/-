document.addEventListener("DOMContentLoaded", () => {
  ...
  // Hero Image Slider - FIXED IMPLEMENTATION
  ...
  function initHeroSlider() {
    ...
    console.log("Initializing hero slider with", heroSlides.length, "slides")
    ...
    heroDots.forEach((dot, index) => {
      dot.addEventListener("click", (e) => {
        ...
        console.log("Hero dot clicked:", index)
        stopHeroAutoSlide()
        showHeroSlide(index)
        startHeroAutoSlide()
      })
  ...
  function startAutoSlide() {
    ...
    autoSlideInterval = setInterval(() => {
      if (!document.hidden) {
        // Only advance slides when page is visible
        showSlide(currentSlide + 1)
      }
    }, slideDuration)
  }
  ...
  if (testimonialCards.length > 0) {
    ...
    setInterval(() => {
      if (!document.hidden) {
        // Only advance when page is visible
        currentTestimonial = (currentTestimonial + 1) % testimonialCards.length
        showTestimonial(currentTestimonial)
      }
    }, 5000)
...
function extractPrice(container) {
  const priceElement =
    container.querySelector(".new-price") ||
    container.querySelector(".featured-price") ||
    container.querySelector(".current-price")
  if (priceElement) {
    const priceText = priceElement.textContent.trim()
    const priceMatch = priceText.match(/(\d+)/)
    return priceMatch ? Number.parseInt(priceMatch[0]) : 0
  }
  return 0
}
